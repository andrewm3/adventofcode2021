...
for n in nums:
    ...
    losers = filter(lambda b: not any([all([y for (x, y) in row]) for row in b]) and
                              not any([all([y for (x, y) in col]) for col in zip(*b)]),
                    boards)
    if len(boards) == 1 and len(losers) == 0:
        score = n * sum([x for row in boards[0] for (x, y) in row if not y])
        break
    boards = losers
